...
def draw(surf, paddles, ball, left_score, right_score):
    surf.fill(BLACK)

    left_score_text = SCORE_FONT.render(f'{left_score}', True, WHITE)
    right_score_text = SCORE_FONT.render(f'{right_score}', True, WHITE)
    surf.blit(left_score_text, (WIDTH//4-left_score_text.get_width()//2, 20))
    surf.blit(right_score_text, (WIDTH*3//4-right_score_text.get_width()//2, 20))
...
def handle_paddle_movement(keys, left_paddle, right_paddle):
    if keys[pygame.K_w] and left_paddle.y - left_paddle.VEL >= 0:
        left_paddle.move()
    elif keys[pygame.K_s] and left_paddle.y + left_paddle.VEL + left_paddle.height <= HEIGHT:
